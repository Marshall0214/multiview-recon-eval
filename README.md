...
> 状态：全部实验已完成：合成数据主实验（M0–M3）、真实木椅测量（M4），以及 Tanks and Temples 真实场景的补充实验。所有数字都来自 `results/` 下的原始结果，没有预估值。
...
| **真实木椅：手机 + A4 标定板量尺寸** | 115/115 张注册，ArUco 对齐 RMS **0.66 mm**；按卷尺口径测 10 个尺寸，**平均绝对误差 9.4 mm，最大 16 mm** |
...
| Q4 | 整条流程能否在真实家具上跑通？尺寸精度如何？ | 能；10 个尺寸平均误差 9.4 mm。但预先登记的测量版本在反光地面上失效，见 §4.7 |
...
**真实场景补充实验（Tanks and Temples Meetingroom，激光扫描真值）**：用同样的 3DGS 训练，留出帧的 PSNR 为 24.9，但直接由渲染深度得到的点云 F-score 只有 0.059（τ = 1 cm，官方评测流程），precision 仅 3.5%，重建点大多是漂浮点。这个结论在定性上与合成实验一致。需要注意：由于 TSDF 在 64 GB 内存下无法完成融合，这里改用了不做任何滤波的深度反投影，**所以绝对数值不能和文献中的 3DGS 结果直接比较**。另外，官方 ICP 的尺度漂移问题、以及用稀疏点检验配准的过程，都记录在 [results/tnt_meetingroom.md](results/tnt_meetingroom.md)。
...
**真实木椅**（详见 [results/m4_real_chair.md](results/m4_real_chair.md)）：iPhone 12 拍摄 115 张，A4 标定板平放在地面上（打印缩放 99.6%，用卷尺量整块图案得到）。

| 步骤 | 结果 |
|---|---|
| COLMAP 注册 | 115/115 张 |
| ArUco 对齐 | 三角化 44 个角点，RMS **0.66 mm** |

| 尺寸 | 卷尺 | v1（预先登记） | v2（事后修复） |
|---|---|---|---|
| 高 | 804* | +51 mm | +15 mm |
| 宽 | 584 | +152 mm | −11 mm（扶手处，同口径） |
| 进深 | 496 | +147 mm | −11 mm（座面处，同口径） |

\* 第一次测量取的是靠背中间（804 mm）；靠背顶部是弧形，两侧立柱高 829 mm。

- **v1 失效的原因**：地面是高反光的抛光瓷砖，3DGS 把倒影重建成了"地面以下的几何"，RANSAC 拟合出的地面被拉低了 3–5 cm；贴地的噪点又和椅腿连成一片，把占地范围撑大了约 15 cm。这两种情况在模拟场景里都没有出现。
- **v2 的修复**：直接用 ArUco 标定板平面作为地面，去掉离地 3 cm 以内的点，占地尺寸改用分位数，并选取中心附近最大的点簇。v2 是看过结果后设计的，所以又回到模拟数据上复查了一遍，三项误差都在 2.4% 以内。
- **口径问题**：v2 最初"进深 +46 mm"的误差，原因是卷尺量的是座面前后，而程序量的是整体占地，包括后倾靠背伸出的约 7 cm。后来补了一套 10 个尺寸的卷尺测量规范，程序按完全相同的定义去量：

| 尺寸 | 卷尺 | 重建 | 误差 |
|---|---|---|---|
| 高（两侧立柱） | 829 | 817 | −12 |
| 座面高 / 扶手高 | 395 / 638 | 399 / 635 | +4 / −3 |
| 扶手外宽 / 前腿外宽 / 座面宽 | 585 / 518 / 521 | 572 / 529 / 523 | −13 / +11 / +2 |
| 座面进深 / 椅腿进深 / 整体进深 | 496 / 478 / 563 | 487 / 494 / 548 | −9 / +16 / −15 |
| 靠背外伸 | 76 | 66 | −10 |

**10 个尺寸的平均绝对误差是 9.4 mm，最大 16 mm**（换一个采样种子是 9.6 mm）。高度系统性偏低约 1 cm，与模拟数据上观察到的"薄边缘被侵蚀"一致。

**经验**：采集规范必须把每个尺寸定义成卷尺真正能量的东西。"占地外包矩形的短边"这种定义，没有人会拿卷尺去量。
